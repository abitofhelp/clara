...
procedure Hello_Clara is
   --  Define error type
   type Error_Kind is (Parse_Error, Range_Error);

   --  Instantiate Result
   package Int_Result is new Clara.Result
...
```ada
package User_Result is new Clara.Result (T => User, E => Error_Kind);
...
```ada
package User_Option is new Clara.Option (T => User);
...
```ada
package String_Or_Int is new Clara.Either (L => String, R => Integer);
...
```ada
with Clara.Try.Map_To_Result_With_Param;
...
package Try_Read is new Clara.Try.Map_To_Result_With_Param
...
```ada
with Clara.Try.To_Option;

function Check_File_Raw (Path : String) return Boolean is ...;

package Try_Check is new Clara.Try.To_Option
  (T        => Boolean,
